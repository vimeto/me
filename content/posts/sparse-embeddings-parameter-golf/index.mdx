...
I kept thinking the embedding matrix was the easy part of [parameter-golf](https://github.com/openai/parameter-golf). It's just a lookup table. You quantise it, you compress it, you ship. Then I spent six weeks on it and shipped nothing.
...
<ParamPlot
  ...
  caption="Each series sweeps vocab 8k→32k. Vertical line is the 16 MB brotli artifact cap. Horizontal line is the locked SP8k+CO INT7 baseline. The slider picks a vocab for the summary readout."
  kind="line"
  xLabel="compressed embed (MB)"
  yLabel="validation BPB"
  params={{ vocab: 8192 }}
  controls={[
    { kind: 'slider', param: 'vocab', label: 'Vocab size', min: 8192, max: 32768, step: 2048, format: 'int', hint: 'SP8k locked baseline; SP12k–SP32k all blow the cap at INT7' },
  ]}
  height={320}
/>
...
<ParamPlot
  ...
  caption="Bars: bits assigned per row (head vs tail). Overlaid line: log-token frequency. Drag the cutoff and the tail-bit slider; the summary updates compressed bytes vs a uniform INT7 reference."
  kind="bar"
  xLabel="row (rank)"
  yLabel="bits"
  ...
  controls={[
    ...
    { kind: 'slider', param: 'tailBits', label: 'Tail bits', min: 2, max: 8, step: 1, format: 'int' },
...
Once the mask was honest, Phase C: SP12k+CO topk-0.10, four seeds (42, 43, 44, 1337), full training. Sidecar TTT BPB landed at {1.0931, 1.0982, 1.1009, 1.0959}. Best 1.0931, mean ~1.097. The range across seeds was 0.0078 — no lottery, no rescue from a lucky init. Decompose it: the bigger vocab buys −0.012, sparse-FP16 costs +0.045, net +0.033 over the SP8k INT7 baseline.
...
I tried stacking sparse-FP16 with the full hparam stack from PR [#1855](https://github.com/openai/parameter-golf/pull/1855) (BETA2=0.99, MLP_CLIP=11.5, EMBED_CLIP=14.0, WARMDOWN_FRAC=0.85, TTT_LORA_RANK=80, the works). Default-recipe sparse-FP16 was around 1.10 BPB. The stacked variant plateaued at 1.32092. The model just sat there at 1.32 ☹️.
